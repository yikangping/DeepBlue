...
def unique_mask_values_DeepBlue(mask_file, mask_dir):
...
class DeepBlueDataset(BasicDataset):
    def __init__(self, dataset_dir='./data/DeepBlue', split='train', logger=None, scale=1.0):
        # Set dataset directory and split.
        self.dataset_dir = dataset_dir
        self.split       = split
        self.logger      = logger
        self.images_dir  = os.path.join(dataset_dir, split)
        self.mask_dir    = os.path.join(dataset_dir, "labels")
        ...
        self.scale=scale
        # self.ids=[i for i in range(len(self.edges_path))]
        logging.info(f'Creating dataset with {len(self.images_path)} examples')
        if split=="train":
            logging.info('Scanning mask files to determine unique values')
            with Pool() as p:
                unique = list(tqdm(
                    p.imap(partial(unique_mask_values_DeepBlue, mask_dir=self.dataset_dir), self.edges_path),
                    total=len(self.edges_path)
                ))
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            # Get edge.
            mask_path = join(self.dataset_dir, self.edges_path[index])
        ...
        image = self.preprocess(self.mask_values, image, self.scale, is_mask=False)
