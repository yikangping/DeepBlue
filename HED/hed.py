...
def main():
    ...
    if not args.test:
        train_dataset = BsdsDataset(dataset_dir=args.dataset, split='train', logger=log)
        train_loader  = DataLoader(train_dataset, batch_size=args.train_batch_size,
                                   num_workers=4, drop_last=True, shuffle=True)

    test_dataset  = BsdsDataset(dataset_dir=args.dataset, split='test', logger=log)
    ...
    if args.test is True:
        ...
        for epoch in range(args.max_epoch):
            ...
            train_epoch_loss = \
                train(train_loader, net, opt, lr_schd, epoch, save_dir=join(output_dir, 'epoch-{}-train'.format(epoch)), log=log)
...
def train(train_loader, net, opt, lr_schd, epoch, save_dir, log):
    """ Training procedure. """
    ...
    for batch_index, (images, edges) in enumerate(tqdm(train_loader)):
        ...
        # log.write("preds shape: {}".format([pred.shape for pred in preds_list]))
        # log.write("edges shape: {}\n".format(edges.shape))
        # print([pred.shape for pred in preds_list])
